...
router.post('/update', async (request, response) => {
    try {
        ...
        var packagesToDeactivate = objectToSave.Packages.filter(x => x.IsDeleted == true);
        var servicesToDeactivate = objectToSave.Services.filter(x => x.IsDeleted == true || x.IsChanged);
        var discountsToDeactivate = objectToSave.Discounts.filter(x => x.IsDeleted == true);
        var packagesToAdd = objectToSave.Packages.filter(x => x.IsNew == true);
        var discountsToAdd = objectToSave.Discounts.filter(x => x.IsNew == true && !x.IsDeleted);
        var servicesToAdd = objectToSave.Services.filter(x => x.IsNew == true || x.IsChanged);
        var promises = [];
        ...
        packagesToDeactivate.forEach(element => {
            promises.push(
                pool.request()
                    ...
                    .query("exec [dbo].[deactivatePackageForPerson] @PackageId=@id, @PersonId=@person")
            );
        });
        ...
        servicesToDeactivate.forEach(element => {
            promises.push(
                pool.request()
                    .input('id', element.Id)
                    .input('person', objectToSave.PersonId)
                    .query("exec [dbo].[deactivateServiceForPerson] @ServiceId=@id, @PersonId=@person")
            );
        });
        ...
        discountsToDeactivate.forEach(element => {
            promises.push(
                pool.request()
                    .input('id', element.Id)
                    .input('person', objectToSave.PersonId)
                    .query("exec [dbo].[deactivateDiscountForPerson] @DiscountId=@id, @PersonId=@person")
            );
        });
        ...
        packagesToAdd.forEach(element => {
            promises.push(
                pool.request()
                    ...
                    .input('person', objectToSave.PersonId)
                    .query("exec [dbo].[insertPackageForPerson] @PackageId=@packageId, @PersonId=@person")
            );
        });
        ...
        discountsToAdd.forEach(element => {
            promises.push(
                pool.request()
                    .input('id', element.Id)
                    .input('person', objectToSave.PersonId)
                    .query("exec [dbo].[insertDiscountForPerson] @DiscountId=@id, @PersonId=@person")
            );
        });
