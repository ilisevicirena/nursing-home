...
router.get("/", async (request, response) => {
  try {
    const pool = await db;
    const result = await pool.request().query("EXEC [dbo].[getPackages]");
        ...
        objectToSave.Services.forEach((element) => {
          promises.push(
            pool
              ...
              .query(
                "exec [dbo].[insertServiceForPackage] @ServiceId=@service, @PackageId=@package, @Quantity=@qty"
              )
          );
        });
      ...
      servicesToAdd.forEach((element) => {
        promises.push(
          pool
            ...
            .input("qty", element.Quantity)
            .query(
              "exec [dbo].[insertServiceForPackage] @ServiceId=@serviceId, @PackageId=@packageId, @Quantity=@qty"
            )
        );
      });
